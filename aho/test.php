...
function change_depart(){
		count = document.getElementById('sel_depart').value;
		var koef = document.getElementById('koef').value;
		
		var fin_count = parseInt(count, 10) + parseInt(koef, 10);
...
<?
	...
	$retval = mysql_query("SELECT count(*) FROM `tab_users` where status = 'Специалист' or status = 'Руководитель' or status = 'Служащий'", $link);
...
<table border=0 width=100%>
	<tr>
		<td align=left>
		<select onchange="change_depart();" id=sel_depart>
			<optgroup label='Итого'>
			<?
				print "<option value='$count_all'>Все подразделения ($count_all чел.)</option>";
			$link = mysql_connect("10.25.8.8", "admin", "conect");
			mysql_query("use aho");
			mysql_query("SET NAMES cp1251");
			$retval = mysql_query("SELECT distinct depart FROM tab_users order by depart", $link);
				while($row = mysql_fetch_array($retval)) {
					$depart = $row[0];
					if($depart != "" and $depart != "производственно-диспетчерская служба") {
						print "<optgroup label='$depart'>";
							$link0 = mysql_connect("10.25.8.8", "admin", "conect");
							mysql_query("use aho");
							mysql_query("SET NAMES cp1251");
							$retval0 = mysql_query("SELECT distinct subdepart FROM tab_users where depart='$depart' order by subdepart", $link0);
							while($row0 = mysql_fetch_array($retval0)) {
								...
								if($subdepart!="") {
									...
									mysql_query("use aho");
									mysql_query("SET NAMES cp1251");
									$retval1 = mysql_query("SELECT count(*) FROM tab_users where subdepart='$subdepart'", $link1);
									while($row1 = mysql_fetch_array($retval1)) {
										$n_count = $row1[0];
									}
									print "<option style=\"Color:Black;\" value=$n_count>$subdepart ($n_count чел.)</option>";
								}
							}
							mysql_close($link0);
...
<br><br><br><a href='index.php' title='' class='button green'>На главную</a>
