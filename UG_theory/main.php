...
function show_dives(){
	var img_top =  $("#rrr").offset().top; // 20
	var img_left =  $("#rrr").offset().left; // 289
	var off_top=0;
	var off_left=0;
	
	$("div[id*='shit']").each(function(i,elem) {
		off_top = $(elem).offset().top + img_top;
		off_left = $(elem).offset().left + img_left;
		$(elem).offset({top:off_top,left:off_left});
		$(elem).show();
		$(elem).fadeTo(0, 0.5);
	});
...
<center><img src=menu_logo.jpg onmouseover=show_dives(); onmouseout=hide_dives(); style="cursor:pointer;"></center><br>
<?
$back=0;
$link = mysql_connect("10.25.8.8", "admin", "conect");
mysql_query("use UG");
...
$retval = mysql_query("SELECT * FROM `ug_theory_options_$sys_name` order by shema", $link);
	...
	//$html = "<br><br><input type='submit' value='TEST' style='background: url(background.jpg);font-size:17;font-family:Times;width:270px;height:45px;border:1px solid white;'>";
	//print "$html";
mysql_close($link);
?>
...
$retval = mysql_query("SELECT * FROM `ug_theory_$sys_name` where shema='$shema'", $link);
	while($row = mysql_fetch_array($retval)) {
		...
		//KepLer
		$html.= "<div style='display:none;left:$l;top:$t;width:$w;height:$h;position:absolute;' ttt='$t' lll='$l' id='shit_$id'><table border=2 bordercolor=green width=100% height=100% style='border-style:solid;background-color:green ' ><tr><td></td></tr></table></div>";
		//$html.= "<span style='display:block;left:$l;top:$t;width:$w;height:$h;' id='$kvadrati_name'><table border=2 bordercolor=green width=100% height=100% style='border-style:dashed;opacity: 0.5' ><tr><td></td></tr></table></div>";
		print "$html";
	} 
mysql_close($link);
