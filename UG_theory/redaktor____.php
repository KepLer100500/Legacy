...
$kl=2;
$retval0 = mysql_query("SELECT distinct shema FROM ug_theory order by shema", $link);
while($row0 = mysql_fetch_array($retval0)) {
	$name_sss = $row0[0];
		$retval1 = mysql_query("SELECT id FROM ug_theory where shema='$name_sss'", $link);
...
$ret = mysql_query("SELECT parent_img FROM ug_theory order by id", $link);
	while($r = mysql_fetch_array($ret)) {
		$last_img=$r[0];
	}

$ret1 = mysql_query("SELECT count(*) FROM ug_theory where parent_img='$last_img'", $link);
...
function go_menu() {
	document.body.innerHTML += "<form action='menu_editor.php' method='post' id='FormExit3'></form>";
	document.getElementById("FormExit3").submit();
}

function ins_href() {
	var name_ssilko = document.getElementById("name_ssilko").value;
	var path_ssilko = "/docs/" + document.getElementById('file_doc').options[document.getElementById('file_doc').selectedIndex].text;
...
function delet(id_what){
		if(confirm("Вы точно хотите удалить выбранный объект?")) {
			document.body.innerHTML += "<form action='action.php' method='post' id='Form1'><input type='hidden' id='parent' value='<?echo $parent_img;?>'><input type='hidden' name='act' value='del'><input type='hidden' name='id' value=" + id_what + "></form>";
...
<tr><td colspan=2 align=center background=background3.jpg><font color=white size=5>Редактор</font></td></tr>
...
<select id=file_doc name=file_doc style="width:100">
...
<select id=file_doc_video name=file_doc_video style="width:100">
...
<select id=file_doc_3d name=file_doc_3d style="width:100">
...
mysql_query("use UG");
mysql_query("SET NAMES cp1251");
$retval = mysql_query("SELECT shema FROM `ug_theory_options` where parent_img='$file_p'", $link);
	while($row = mysql_fetch_array($retval)) {
		$shema = $row[0];
	}
...
mysql_query("insert into `ug_theory` values('','$shema','$name_dat','$res_coords','$opis_dat','$file_img','$file_p','$ssilko_all','$ssilko_all_video','$ssilko_all_3d')");
mysql_close($link);	
...
$name_fontanka = $_POST['name_fontanka'];
	$uploaddir = "p_img/";
	mysql_query("insert into `ug_theory_options` values('$name_shema','$temp','$name_fontanka')");
...
foreach (glob("p_img/*") as $filename){
	if(strpos($filename,"jpg") > 0 || strpos($filename,"JPG") > 0 || strpos($filename,"JPEG") > 0 || strpos($filename,"jpeg") > 0){
		...
		mysql_query("use UG");
		mysql_query("SET NAMES cp1251");
		$retval = mysql_query("SELECT count(*) FROM `ug_theory` where parent_img='$massiv'", $link);
		while($row = mysql_fetch_array($retval)) {
			$count=$row[0];
		}
		
		$retval = mysql_query("SELECT coords,id,image FROM `ug_theory` where parent_img='$massiv' order by id", $link);
			$ddd=0;
			//echo "-$count-";
			//416
			while($row = mysql_fetch_array($retval)) {
